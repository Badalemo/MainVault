...
1. I created an instance of profit tracker, so that the lists will not get confused (i.e. the class will have its own independent set of data, which prevents the lists from interfering with each other)
	/list. Going more in-depth for this tracker explanation: In the case where there is more than one class, changes made in one scenario would affect all the others, which would lead to errors and other unwanted results.
